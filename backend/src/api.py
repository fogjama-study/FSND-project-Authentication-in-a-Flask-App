...
def update_drink(jwt, drink_id):
    ...
    try:
        drink_db = Drink.query.filter(Drink.id==int(drink_id)).one_or_none()

        if drink is None:
            abort(404)
        
        if 'title' in drink:
            drink_db.title = drink['title']
            drink_db.update()
        
        if 'recipe' in drink:
            drink_db.recipe = json.dumps(drink['recipe'])
            drink_db.update()

        updated_drink = drink_db.long()
